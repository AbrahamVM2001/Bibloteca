$(function () {
    async function cardsProgramas() {
        console.log("Entras");
        try {
            let peticion = await fetch(servidor + `reportes/infoProgramas/${evento}`);
            ...
            response.forEach((item, index) => {
                jQuery(`
                    <a href="${servidor}reportes/programa/${btoa(btoa(item.id_programa))}/${btoa(item.nombre_programa)}/${btoa(btoa(item.nombre_evento+"-"+item.nombre_programa))}" class="col-sm-12 col-md-12 col-lg-4 col-xl-4 mb-3">
