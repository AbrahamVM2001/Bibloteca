$(function () {
        ...
        if (form[0].checkValidity() === false) {
            ...
            if ($('#idioma').val().length == 0) {
                ...
                if($('#id_idioma').val().length == 0){
                    $.ajax({
                        type: "POST",
                        url: servidor + "admin/RegistroIdioma",
                        dataType: "json",
                    ...
                    $.ajax({
                        type: "POST",
                        url: servidor + "admin/actualizarIdioma",
                        dataType: "json",
                        data: new FormData(form.get(0)),
                        contentType: false,
                        cache: false,
    ...
    async function cardsEventos() {
        try {
            let peticion = await fetch(servidor + `admin/MostrarIdioma`);
    ...
    swalWithBootstrapButtons.fire({
        ...
        reverseButtons: true
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: servidor + `admin/eliminarIdioma/${id_idioma}`,
                    dataType: "json",
                    beforeSend: function () {
                        $("#loading").addClass("loading");
                    },
...
async function buscarIdioma(id_idioma) {
    try {
        let peticion = await fetch(servidor + `admin/buscarIdioma/${id_idioma}`);
        let response = await peticion.json();
