$(function () {
    async function cardsEventos(id_libro) {
        try {
            ...
            let peticion = await fetch(servidor + `admin/viewComentario/${id_libro}`);
    ...
    async function buscarComentarioEditar(id_comentario) {
        try {
            let peticion = await fetch(servidor + `admin/buscarComentarioEditar/${id_comentario}`);
            let response = await peticion.json();
    ...
    function eliminarComentario(id_comentario) {
        $.ajax({
            type: "POST",
            url: servidor + `admin/eliminarComentario/${id_comentario}`,
            dataType: "json",
            beforeSend: function () {
    ...
    async function cardsProgramas(id_libro) {
        console.log("Entras");
        try {
            let peticion = await fetch(servidor + `admin/infoLibro/${id_libro}`);
            let response = await peticion.json();
    ...
    async function loadPdfWithProgress(pdfUrl, embedSelector, progressCallback) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
                    ...
                    loadedPages++;
                    let progress = (loadedPages / totalPages) * 100;
    ...
    $(".btn-Comenario").on("click", function () {
        ...
        if (form[0].checkValidity() === false) {
            ...
            if ($('#comentario').val().length == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: servidor + "admin/RegistroComentario",
                    dataType: "json",
