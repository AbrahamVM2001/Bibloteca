$(function () {
    $(".btn-evento").on("click", function () {
        ...
        if (form[0].checkValidity() === false) {
            ...
            if ($('#nombre').val().length == 0 || $('#APaterno').val().length == 0 || $('#genero').val().length == 0 || $('#email').val().length == 0 || $('#password').val().length == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: servidor + "admin/RegistroUsuario",
                    dataType: "json",
    ...
    async function cardsEventos() {
        try {
            let peticion = await fetch(servidor + `admin/MostrarUsuario`);
    ...
    swalWithBootstrapButtons.fire({
        ...
        reverseButtons: true
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: servidor + `admin/eliminarUsuario/${id_usuario}`,
                    dataType: "json",
                    beforeSend: function () {
                        $("#loading").addClass("loading");
                    },
...
function actualizarUsuario(formData) {
    if ($('#ac_nombre').val().length == 0 || $('#ac_apellido_paterno').val().length == 0 || $('#ac_genero').val().length == 0 || $('#ac_correo').val().length == 0 || $('#ac_password').val().length == 0 || $('#ac_tipo_usuario').val().length == 0 || $('#ac_estatus').val().length == 0) {
        ...
        var usuarioLogeado = $('#id_usuario').val();
        var usuarioEditable = $('#ac_id_usuario').val();
        var estatus = $('#ac_estatus').val();
        if (usuarioEditable == usuarioLogeado && estatus == '0') {
            Swal.fire("No puedes desahabilitarte lo siento");
        } else {
            if (estatus == 0) {
                enviarCorreoInhabilitacion(formData);
            }
            $.ajax({
                type: "POST",
...
function enviarCorreoInhabilitacion(formData) {
    $.ajax({
        type: "POST",
        url: servidor + 'admin/enviocorreoacceso',
        dataType: "json",
        success: function (data) {
            console.log(data);
            Swal.fire({
        ...
        error: function (xhr, status, error) {
            console.log("Error ajax enviarCorreoInhabilitacion");
            console.log(xhr);
            console.log(status);
            console.log(error);
        },
