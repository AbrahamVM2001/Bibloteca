$(function () {
    async function cardsEventos() {
        try {
            let peticion = await fetch(servidor + `usuario/MostrarLibrosView`);
    ...
    async function cardsRomance() {
        try {
            ...
            response.forEach((item, index) => {
                jQuery(`
                    <a href="${servidor}usuario/pdf/${btoa(btoa(item.id_libro))}/${btoa(item.documento)}">
    ...
    async function cardsHarry() {
        try {
            let peticion = await fetch(servidor + `usuario/MostrarLibrosViewHarry`);
            let response = await peticion.json();
            if (response.length == 0) {
            ...
            response.forEach((item, index) => {
                jQuery(`
                    <a href="${servidor}usuario/pdf/${btoa(btoa(item.id_libro))}/${btoa(item.documento)}"><img src="${servidor}${item.Imagen}" class= "img  img-responsive   img-thumbnail"></a>
    ...
    async function cardsASCII() {
        try {
            let peticion = await fetch(servidor + `usuario/MostrarLibrosViewACII`);
            let response = await peticion.json();
            if (response.length == 0) {
