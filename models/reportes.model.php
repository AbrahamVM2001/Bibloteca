...
class ReportesModel extends ModelBase
{
    ...
    public static function temasAsignadosProfesores($idprograma){
        try {
            $con = new Database;
            $query = $con->pdo->prepare("SELECT cfp.fecha_programa,cs.nombre_salon,cc.nombre_capitulo,ca.nombre_actividad,atp.hora_inicial,atp.hora_final,ct.nombre_tema,cm.nombre_modalidad,concat_ws(' ',cpr.siglas_prefijo,cp.nombre_profesor,cp.apellidop_profesor,cp.apellidom_profesor) AS profesor FROM asignacion_temas_programa atp INNER JOIN cat_modalida cm ON cm.id_modalidad = atp.fk_id_modalidad INNER JOIN cat_profesores cp ON cp.id_profesor = atp.fk_id_profesor INNER JOIN cat_prefijos cpr ON cpr.id_prefijo = cp.fk_id_prefijo INNER JOIN cat_temas ct ON ct.id_tema = atp.fk_id_tema INNER JOIN cat_actividades ca ON ca.id_actividad = atp.fk_id_actividad INNER JOIN cat_capitulos cc ON cc.id_capitulo = atp.fk_id_capitulo INNER JOIN cat_salones cs ON cs.id_salon = atp.fk_id_salon INNER JOIN cat_fechas_programa cfp ON cfp.id_fecha_programa = atp.fk_id_fechas WHERE atp.fk_id_programa = :idPrograma AND atp.estatus_asignacion = 1 ORDER BY cfp.fecha_programa,atp.hora_inicial ASC;");
            $query->execute([
